...
\{\+: .callout .note\} {\bfseries{Note\+:}} g\+Mock lives in the {\ttfamily testing} name space. For readability, it is recommended to write {\ttfamily using \+::testing\+::\+Foo;} once in your file before using the name {\ttfamily Foo} defined by g\+Mock. We omit such {\ttfamily using} statements in this section for brevity, but you should do it in your own code.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md119}{}\doxysection{Creating Mock Classes}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md119}
...
\end{DoxyItemize}\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md120}{}\doxysubsection{Dealing with unprotected commas}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md120}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md121}{}\doxysubsection{Mocking Private or Protected Methods}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md121}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md122}{}\doxysubsection{Mocking Overloaded Methods}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md122}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md123}{}\doxysubsection{Mocking Class Templates}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md123}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md124}{}\doxysubsection{Mocking Free Functions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md124}
...
If you are concerned about the performance overhead incurred by virtual functions, and profiling confirms your concern, you can combine this with the recipe for \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_MockingNonVirtualMethods}{mocking non-\/virtual methods}}.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md125}{}\doxysubsection{Old-\/\+Style $<$tt$>$\+MOCK\+\_\+\+METHODn$<$/tt$>$ Macros}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md125}
...
If a method of {\ttfamily mock\+\_\+foo} other than {\ttfamily Do\+This()} is called, you will get a warning. However, if you rewrite your test to use {\ttfamily Nice\+Mock\texorpdfstring{$<$}{<}\mbox{\hyperlink{classMockFoo}{Mock\+Foo}}\texorpdfstring{$>$}{>}} instead, you can suppress the warning\+:
...
\{\+: .callout .note\} NOTE\+: {\ttfamily Nice\+Mock} and {\ttfamily Strict\+Mock} only affects {\itshape uninteresting} calls (calls of {\itshape methods} with no expectations); they do not affect {\itshape unexpected} calls (calls of methods with expectations, but they don\textquotesingle{}t match). See \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_uninteresting-vs-unexpected}{Understanding Uninteresting vs Unexpected Calls}}.

There are some caveats though (sadly they are side effects of C++\textquotesingle{}s limitations)\+:


\begin{DoxyEnumerate}
\item {\ttfamily Nice\+Mock\texorpdfstring{$<$}{<}\mbox{\hyperlink{classMockFoo}{Mock\+Foo}}\texorpdfstring{$>$}{>}} and {\ttfamily Strict\+Mock\texorpdfstring{$<$}{<}\mbox{\hyperlink{classMockFoo}{Mock\+Foo}}\texorpdfstring{$>$}{>}} only work for mock methods defined using the {\ttfamily MOCK\+\_\+\+METHOD} macro {\bfseries{directly}} in the {\ttfamily \mbox{\hyperlink{classMockFoo}{Mock\+Foo}}} class. If a mock method is defined in a {\bfseries{base class}} of {\ttfamily \mbox{\hyperlink{classMockFoo}{Mock\+Foo}}}, the \char`\"{}nice\char`\"{} or \char`\"{}strict\char`\"{} modifier may not affect it, depending on the compiler. In particular, nesting {\ttfamily Nice\+Mock} and {\ttfamily Strict\+Mock} (e.\+g. {\ttfamily Nice\+Mock\texorpdfstring{$<$}{<}Strict\+Mock\texorpdfstring{$<$}{<}\mbox{\hyperlink{classMockFoo}{Mock\+Foo}}\texorpdfstring{$>$}{>} \texorpdfstring{$>$}{>}}) is {\bfseries{not}} supported.
\item {\ttfamily Nice\+Mock\texorpdfstring{$<$}{<}\mbox{\hyperlink{classMockFoo}{Mock\+Foo}}\texorpdfstring{$>$}{>}} and {\ttfamily Strict\+Mock\texorpdfstring{$<$}{<}\mbox{\hyperlink{classMockFoo}{Mock\+Foo}}\texorpdfstring{$>$}{>}} may not work correctly if the destructor of {\ttfamily \mbox{\hyperlink{classMockFoo}{Mock\+Foo}}} is not virtual. We would like to fix this, but it requires cleaning up existing tests.
\end{DoxyEnumerate}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md126}{}\doxysubsection{Alternative to Mocking Concrete Classes}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md126}
...
Instead, you can define a {\ttfamily File\+Ops} interface and an {\ttfamily IOOps} interface and split {\ttfamily System}\textquotesingle{}s functionalities into the two. Then you can mock {\ttfamily IOOps} without mocking {\ttfamily File\+Ops}.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md127}{}\doxysubsection{Delegating Calls to a Real Object}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md127}
...
You can use the {\itshape delegating-\/to-\/real} technique to ensure that your mock has the same behavior as the real object while retaining the ability to validate calls. This technique is very similar to the \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_DelegatingToFake}{delegating-\/to-\/fake}} technique, the difference being that we use a real object instead of a fake. Here\textquotesingle{}s an example\+:
...
(Why don\textquotesingle{}t we just write {\ttfamily \{ return foo.\+Concrete(str); \}}? If you do that, {\ttfamily Mock\+Foo\+::\+Concrete()} will be called (and cause an infinite recursion) since {\ttfamily Foo\+::\+Concrete()} is virtual. That\textquotesingle{}s just how C++ works.)\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md129}{}\doxysection{Using Matchers}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md129}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md130}{}\doxysubsection{Matching Argument Values Exactly}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md130}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md131}{}\doxysubsection{Using Simple Matchers}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md131}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md132}{}\doxysubsection{Performing Different Actions Based on the Arguments}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md132}
...
Now, if {\ttfamily foo.\+Do\+This()} is called with a value less than 5, `\textquotesingle{}a'{\ttfamily will be returned; otherwise}\textquotesingle{}b\textquotesingle{}\`{} will be returned.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md133}{}\doxysubsection{Matching Multiple Arguments as a Whole}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md133}
...
Note that if you want to pass the arguments to a predicate of your own (e.\+g. {\ttfamily .With(Args\texorpdfstring{$<$}{<}0, 1\texorpdfstring{$>$}{>}(Truly(\&My\+Predicate)))}), that predicate MUST be written to take a {\ttfamily std\+::tuple} as its argument; g\+Mock will pass the {\ttfamily n} selected arguments as {\itshape one} single tuple to the predicate.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md134}{}\doxysubsection{Using Matchers as Predicates}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md134}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md135}{}\doxysubsection{Using Matchers in googletest Assertions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md135}
See \href{reference/assertions.md\#EXPECT_THAT}{\texttt{ {\ttfamily EXPECT\+\_\+\+THAT}}} in the Assertions Reference.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md136}{}\doxysubsection{Using Predicates as Matchers}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md136}
...
Note that the predicate function / functor doesn\textquotesingle{}t have to return {\ttfamily bool}. It works as long as the return value can be used as the condition in in statement {\ttfamily if (condition) ...}.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md137}{}\doxysubsection{Matching Arguments that Are Not Copyable}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md137}
...
Remember\+: if you do this, don\textquotesingle{}t change {\ttfamily bar} after the {\ttfamily EXPECT\+\_\+\+CALL()}, or the result is undefined.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md138}{}\doxysubsection{Validating a Member of an Object}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md138}
...
What if you want to validate more than one members at the same time? Remember that there are \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_CombiningMatchers}{\`{}\+All\+Of()\`{} and \`{}\+All\+Of\+Array()\`{}}}.
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md139}{}\doxysubsection{Validating the Value Pointed to by a Pointer Argument}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md139}
...
Most matchers can be simply defined using \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_NewMatchers}{the MATCHER$\ast$ macros}}, which are terse and flexible, and produce good error messages. However, these macros are not very explicit about the interfaces they create and are not always suitable, especially for matchers that will be widely reused.
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md140}{}\doxysubsection{Matching Containers}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md140}
...
\end{DoxyItemize}\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md141}{}\doxysubsection{Sharing Matchers}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md141}
...
This requirement must be satisfied no matter how a matcher is defined (e.\+g., if it is one of the standard matchers, or a custom matcher). In particular, a matcher can never call a mock function, as that will affect the state of the mock object and g\+Mock.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md142}{}\doxysection{Setting Expectations}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md142}
...
If you are bothered by the \char`\"{}\+Uninteresting mock function call\char`\"{} message printed when a mock method without an {\ttfamily EXPECT\+\_\+\+CALL} is called, you may use a {\ttfamily Nice\+Mock} instead to suppress all such messages for the mock object, or suppress the message for specific methods by adding {\ttfamily EXPECT\+\_\+\+CALL(...).Times(\+Any\+Number())}. DO NOT suppress it by blindly adding an {\ttfamily EXPECT\+\_\+\+CALL(...)}, or you\textquotesingle{}ll have a test that\textquotesingle{}s a pain to maintain.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md143}{}\doxysubsection{Ignoring Uninteresting Calls}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md143}
If you are not interested in how a mock method is called, just don\textquotesingle{}t say anything about it. In this case, if the method is ever called, g\+Mock will perform its default action to allow the test program to continue. If you are not happy with the default action taken by g\+Mock, you can override it using {\ttfamily Default\+Value\texorpdfstring{$<$}{<}T\texorpdfstring{$>$}{>}::Set()} (described \mbox{\hyperlink{md_build__deps_googletest_src_docs_reference_mocking_DefaultValue}{here}}) or {\ttfamily ON\+\_\+\+CALL()}.

Please note that once you expressed interest in a particular mock method (via {\ttfamily EXPECT\+\_\+\+CALL()}), all invocations to it must match some expectation. If this function is called but the arguments don\textquotesingle{}t match any {\ttfamily EXPECT\+\_\+\+CALL()} statement, it will be an error.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md144}{}\doxysubsection{Disallowing Unexpected Calls}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md144}
...
A call to {\ttfamily foo.\+Bar()} that doesn\textquotesingle{}t match any of the {\ttfamily EXPECT\+\_\+\+CALL()} statements will be an error.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_uninteresting-vs-unexpected}{}\doxysubsection{Understanding Uninteresting vs Unexpected Calls}\label{md_build__deps_googletest_src_docs_gmock_cook_book_uninteresting-vs-unexpected}
...
For more on uninteresting calls, nice mocks, and strict mocks, read \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_NiceStrictNaggy}{"{}The Nice, the Strict, and the Naggy"{}}}.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_ParameterlessExpectations}{}\doxysubsection{Ignoring Uninteresting Arguments}\label{md_build__deps_googletest_src_docs_gmock_cook_book_ParameterlessExpectations}
...
This functionality is only available when a method is not overloaded; to prevent unexpected behavior it is a compilation error to try to set an expectation on a method where the specific overload is ambiguous. You can work around this by supplying a \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_SimplerInterfaces}{simpler mock interface}} than the mocked class provides.

This pattern is also useful when the arguments are interesting, but match logic is substantially complex. You can leave the argument list unspecified and use Save\+Arg actions to \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_SaveArgVerify}{save the values for later verification}}. If you do that, you can easily differentiate calling the method the wrong number of times from calling it with the wrong arguments.\hypertarget{md_build__deps_googletest_src_docs_gmock_for_dummies_OrderedCalls}{}\doxysubsection{Expecting Ordered Calls}\label{md_build__deps_googletest_src_docs_gmock_for_dummies_OrderedCalls}
...
This means that A must occur before B and C, and C must occur before D. There\textquotesingle{}s no restriction about the order other than these.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md145}{}\doxysubsection{Controlling When an Expectation Retires}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md145}
...
Here \#2 can be used only once, so if you have two warnings with the message {\ttfamily \char`\"{}\+File too large.\char`\"{}}, the first will match \#2 and the second will match \#1 -\/ there will be no error.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md146}{}\doxysection{Using Actions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md146}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md147}{}\doxysubsection{Returning References from Mock Methods}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md147}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md148}{}\doxysubsection{Returning Live Values from Mock Methods}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md148}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md149}{}\doxysubsection{Combining Actions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md149}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_SaveArgVerify}{}\doxysubsection{Verifying Complex Arguments}\label{md_build__deps_googletest_src_docs_gmock_cook_book_SaveArgVerify}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md150}{}\doxysubsection{Changing a Mock Object\textquotesingle{}s Behavior Based on the State}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md150}
...
Please note that changing the default value for a type can make your tests hard to understand. We recommend you to use this feature judiciously. For example, you may want to make sure the {\ttfamily Set()} and {\ttfamily Clear()} calls are right next to the code that uses your mock.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md151}{}\doxysubsection{Setting the Default Actions for a Mock Method}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md151}
...
\end{DoxyItemize}\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md152}{}\doxysubsection{Using Functions with Extra Info as Actions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md152}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md153}{}\doxysubsection{Invoking a Function/\+Method/\+Functor/\+Lambda/\+Callback Without Arguments}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md153}
...
\end{DoxyItemize}\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md154}{}\doxysubsection{Invoking an Argument of the Mock Function}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md154}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md155}{}\doxysubsection{Ignoring an Action\textquotesingle{}s Result}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md155}
...
Note that you {\bfseries{cannot}} use {\ttfamily Ignore\+Result()} on an action that already returns {\ttfamily void}. Doing so will lead to ugly compiler errors.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_SelectingArgs}{}\doxysubsection{Selecting an Action\textquotesingle{}s Arguments}\label{md_build__deps_googletest_src_docs_gmock_cook_book_SelectingArgs}
...
\end{DoxyItemize}\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md156}{}\doxysubsection{Ignoring Arguments in Action Functions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md156}
The \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_SelectingArgs}{selecting-\/an-\/action\textquotesingle{}s-\/arguments}} recipe showed us one way to make a mock function and an action with incompatible argument lists fit together. The downside is that wrapping the action in {\ttfamily With\+Args\texorpdfstring{$<$}{<}...\texorpdfstring{$>$}{>}()} can get tedious for people writing the tests.
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md157}{}\doxysubsection{Sharing Actions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md157}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md158}{}\doxysubsection{Testing Asynchronous Behavior}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md158}
...
\{\+: .callout .note\} Note\+: this example has a downside\+: namely, if the expectation is not satisfied, our test will run forever. It will eventually time-\/out and fail, but it will take longer and be slightly harder to debug. To alleviate this problem, you can use {\ttfamily Wait\+For\+Notification\+With\+Timeout(ms)} instead of {\ttfamily Wait\+For\+Notification()}.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md159}{}\doxysection{Misc Recipes on Using g\+Mock}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md159}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md160}{}\doxysubsection{Mocking Methods That Use Move-\/\+Only Types}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md160}
...
If you are not happy with the default action, you can tweak it as usual; see \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cheat_sheet_OnCall}{Setting Default Actions}}.
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md161}{}\doxysubsection{Making the Compilation Faster}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md161}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md162}{}\doxysubsection{Forcing a Verification}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md162}
...
Also, remember that {\ttfamily Default\+Value\texorpdfstring{$<$}{<}T\texorpdfstring{$>$}{>}} is a global resource that potentially affects {\itshape all} living mock objects in your program. Naturally, you won\textquotesingle{}t want to mess with it from multiple threads or when there still are mocks in action.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md164}{}\doxysubsection{Controlling How Much Information g\+Mock Prints}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md164}
...
Now, judiciously use the right flag to enable g\+Mock serve you better!\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md165}{}\doxysubsection{Gaining Super Vision into Mock Calls}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md165}
...
If you are interested in the mock call trace but not the stack traces, you can combine {\ttfamily -\/-\/gmock\+\_\+verbose=info} with {\ttfamily -\/-\/gtest\+\_\+stack\+\_\+trace\+\_\+depth=0} on the test command line.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md166}{}\doxysubsection{Running Tests in Emacs}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md166}
...
Then you can type {\ttfamily M-\/m} to start a build (if you want to run the test as well, just make sure {\ttfamily foo\+\_\+test.\+run} or {\ttfamily runtests} is in the build command you supply after typing {\ttfamily M-\/m}), or {\ttfamily M-\/up}/{\ttfamily M-\/down} to move back and forth between errors.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md167}{}\doxysection{Extending g\+Mock}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md167}
...
\{\+: .callout .warning\} WARNING\+: g\+Mock does not guarantee when or how many times a matcher will be invoked. Therefore, all matchers must be functionally pure. See \mbox{\hyperlink{md_build__deps_googletest_src_docs_gmock_cook_book_PureMatchers}{this section}} for more details.
...
\{\+: .callout .tip\} Tip\+: for convenience, {\ttfamily Match\+And\+Explain()} can take a {\ttfamily Match\+Result\+Listener$\ast$} instead of {\ttfamily std\+::ostream$\ast$}.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md170}{}\doxysubsection{Writing New Polymorphic Matchers}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md170}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md171}{}\doxysubsection{Legacy Matcher Implementation}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md171}
...
Like in a monomorphic matcher, you may explain the match result by streaming additional information to the {\ttfamily listener} argument in {\ttfamily Match\+And\+Explain()}.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md172}{}\doxysubsection{Writing New Cardinalities}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md172}
...
Typically the {\ttfamily Once\+Action} and {\ttfamily Action} templates need not be referenced directly in your actions\+: a struct or class with a call operator is sufficient, as in the examples above. But fancier polymorphic actions that need to know the specific return type of the mock function can define templated conversion operators to make that possible. See {\ttfamily \mbox{\hyperlink{gmock-actions_8h_source}{gmock-\/actions.\+h}}} for examples.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md173}{}\doxysubsubsection{Legacy macro-\/based Actions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md173}
...
\end{longtabu}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md174}{}\doxysubsubsection{Legacy macro-\/based parameterized Actions}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md174}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md175}{}\doxysubsection{Restricting the Type of an Argument or Parameter in an ACTION}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md175}
...
where {\ttfamily Static\+Assert\+Type\+Eq} is a compile-\/time assertion in googletest that verifies two types are the same.\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md176}{}\doxysubsection{Writing New Action Templates Quickly}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md176}
...
Are we using a single-\/template-\/parameter action where {\ttfamily bool} refers to the type of {\ttfamily x}, or a two-\/template-\/parameter action where the compiler is asked to infer the type of {\ttfamily x}?\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md177}{}\doxysubsection{Using the ACTION Object\textquotesingle{}s Type}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md177}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md178}{}\doxysubsection{Teaching g\+Mock How to Print Your Values}\label{md_build__deps_googletest_src_docs_gmock_cook_book_autotoc_md178}
